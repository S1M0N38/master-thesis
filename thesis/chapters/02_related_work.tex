...
detection and hardcoded pattern recognition. The raw image of the digit is % FIX: is?
...
be trained. Connections between layers, i.e.\ the topology of the network, were % FIX: curly line
...
performance and, as of the time of writing, cannot run on consumer or low-power % FIX: run -> train
...
no inherent structure or relationship between them. Conversely, having richer % FIX: more
...
\paragraph{Hierarchies (trees)} % FIX: ?
...
into labels, into the loss function, or by designing a custom architecture. Of % FIX: Citation needed. See later.
course, the line between custom loss functions and custom architectures is % FIX: Of course = condescending tone?
...
% FIX: undercurl, fine help coarse cls was a main result
